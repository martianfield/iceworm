...
describe('Array Validator', () => {
  ...
  it('Not An Array', () => {
    ...
    let validation = iceworm.validators.array('not an array', fieldInfo)
  ...
  it('Items not correct type', () => {
    let fieldInfo = FieldInfo.create('', "int[]")
    let validation = iceworm.validators.array([1, 2, "not a number"], fieldInfo)
    validation.valid.should.equal(false)
    validation.errors.length.should.equal(1)
    validation.errors[0].reason.should.equal("type")
  })
  ...
  it('All good', () => {
    let fieldInfo = FieldInfo.create('', "int[]")
    let validation = iceworm.validators.array([1, 2, 3], fieldInfo)
